...
@bookings_bp.route("/<int:booking_id>/extend", methods=["PATCH"])
@jwt_required()
@require_role("customer")
def extend_booking(booking_id):
    ...
    if booking.status not in ("BOOKED", "PAID", "PICKED_UP"):
        return jsonify({"error": f"Cannot extend — booking status is '{booking.status}'"}), 400

    try:
        new_end = datetime.fromisoformat(data["new_end_date"])
    except (ValueError, TypeError):
        return jsonify({"error": "Invalid date format — use ISO-8601"}), 400

    if new_end <= booking.end_date:
        return jsonify({"error": "new_end_date must be after current end_date"}), 400

    # Check for conflicts in the extended period
    conflict = check_availability(booking.vehicle_id, booking.end_date, new_end)
    if conflict:
        return jsonify({"error": "Vehicle is booked during the extended period"}), 409

    # Recalculate cost for the full new duration
    vehicle = Vehicle.query.get(booking.vehicle_id)
    new_total = calculate_price(vehicle, booking.start_date, new_end)
